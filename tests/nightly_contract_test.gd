...
func _test_weapon_model_and_transaction() -> void:
	...
	for weapon_id in two_handed_ids:
		for offhand_id in offhand_ids:
			...
			var rejection_before := var_to_bytes(weapon_first.to_snapshot_data())
			var matrix_rejected := weapon_first.equip_from_inventory(second_offhand_key, "left_hand")
			_expect(
				matrix_rejected.get("reason", "") == "two_handed_offhand_blocked"
				and var_to_bytes(weapon_first.to_snapshot_data()) == rejection_before,
				"Offhand %s after two-handed %s must reject byte/state-exactly" % [offhand_id, weapon_id],
			)
	...
	var rejected := state.equip_from_inventory(lamp, "left_hand")
	_expect(
		rejected.get("reason", "") == "two_handed_offhand_blocked"
		and state.to_snapshot_data() == before,
		"Offhand equip against a two-hander must reject without any state mutation",
	)
	var impossible := state.to_snapshot_data()
	impossible.loadout.left_hand = lamp
	_expect(
	...
	_expect(bool(state.unequip("right_hand").get("ok", false)), "Removing a two-hander must succeed")
	_expect(
		bool(state.equip_from_inventory(lamp, "left_hand").get("ok", false)),
		"Removing the two-hander must free the real offhand",
	)
...
func _test_camp_and_soul_contract() -> void:
	...
	_expect(
		state.camp_upgrades.keys().size() == 12
		and state.camp_upgrades.keys().all(func(id): return GameRules.CAMP_UPGRADES.has(id)),
		"Camp state must contain exactly the stable 12-entry registry",
	)
	var before := state.to_snapshot_data()
...
func _continue_test_save_boundary() -> void:
	...
	_expect(bool(first.get("ok", false)), "Current v17 fixture must save")
	state.carried_souls = 1
	var second := SaveSystem.save_slot(state, "backup-case", "overwrite", SAVE_ROOT, 101)
	_expect(bool(second.get("ok", false)), "Second v17 fixture must create a backup")
	...
	_expect(
		...
		and bool(loaded.get("write_locked", false)) and loaded.version == 17,
		"v16 primary with a valid v17 backup must load the backup under write lock",
	)

	var legacy := SaveSystem.save_slot(state, "legacy-16", "overwrite", SAVE_ROOT, 90)
	_expect(bool(legacy.get("ok", false)), "Legacy fixture must start as valid v17")
	...
	_expect(bool(bad_backup_save.get("ok", false)), "Invalid-backup fixture must publish a v17 primary")
	...
	_expect(
		...
		"A valid v17 primary plus unsupported backup must reject without changing family bytes",
	)
	_expect(
	...
	_expect(bool(pending_save.get("ok", false)), "Existing-temporary fixture must publish a v17 primary")
	...
	for malformed_version in ["17", 17.5, 18]:
...
func _test_camp_runtime_assets() -> void:
	...
	for module_id in expected_order:
		...
		_expect(components.size() == 1, "Each camp module must own one isolated atlas cell: %s" % module_id)
	...
	for service_id in ["crusher", "whetstone", "ritual_table", "kettle"]:
		_expect(
